...
#define ERR_IN_VEHICLE "Le remplissage a échoué ! Vous ne pouvez faire ça dans un véhicule"
#define ERR_TOO_FAR_AWAY "Le remplissage a échoué ! Vous êtes trop loin"
#define ERR_CANCELLED "Le remplissage a été annulé !"
...
_checks = {
	...
	switch (true) do {
		...
		default {
			_text = format["Remplissage du bidon d'essence %1%2", round(100 * _progress), "%"];
			_failed = false;
		};
	};
	[_failed, _text];
...
if (_success) then {
	...
	["Bidon d'essence rempli !", 5] call mf_notify_client;
};
false;
