...
[MF_ITEMS_SPAWN_BEACON, "Balise de spawn", _deploy, "Land_Sleeping_bag_folded_F", _icon, 1] call mf_inventory_create;
...
_pack =[format ["<img image='%1'/> Repack la balise", _icon], path("pack.sqf"), [], 1, true, false, "", _condition];
["beacon-pack", _pack] call mf_player_actions_set;

_condition = "'' == [] call mf_items_spawn_beacon_can_steal;";
_steal = [format ["<img image='%1'/> Voler la balise", _icon], path("steal.sqf"), [], 1, true, false, "", _condition];
["beacon-steal", _steal] call mf_player_actions_set;

_condition = "'' == [] call mf_items_spawn_beacon_can_pack && {playerSide != independent}";
_pack =[format ["<img image='%1'/> Changer le clan de la balise", _icon], path("toggle_spawn_permissions.sqf"), [], 1, true, false, "", _condition];
