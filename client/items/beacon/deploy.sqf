...
#define ERR_IN_VEHICLE "L'action a échoué , vous ne pouvez faire ça dans un véhicule"
...
_hasFailed = {
	...
	switch (true) do {
		...
		default {
			_text = format["Déploiement %1%2", round(_progress*100), "%"];
			_failed = false;
		};
	};
	[_failed, _text];
...
if (_success) then {
	...
	["Vous avez déployé la balise !", 5] call mf_notify_client;
};
_success;
