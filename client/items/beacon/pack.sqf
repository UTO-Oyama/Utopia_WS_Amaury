...
#define ERR_CANCELLED "Arrêt du remballage du spawn beacon"
#define ERR_TOO_FAR_AWAY "Vous avez besoin d'être au moins à 5 mètres de la balise"
#define ERR_SOMEONE_ELSE_IS_PACKING "Quelqu'un a déjà pris la balise"
...
_hasFailed = {
	...
	switch (true) do {
		...
		default {
			_text = format["Remballage %1%2", round(_progress*100), "%"];
			_failed = false;
		};
	};
	[_failed, _text];
...
if (_success) then {
	...
	["Vous avez remballé la balise", 5] call mf_notify_client;
};
