...
if (_success) then {
	if (_currentGroupOnlyState) then {
		["La balise de spawn est maintenant disponible pour votre équipe", 5] call mf_notify_client;
		_beacon setVariable ['groupOnly', false, true];
	} else {
		["La balise de spawn est maintenant disponible pour votre groupe", 5] call mf_notify_client;
