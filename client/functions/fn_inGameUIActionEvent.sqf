	...
	switch (true) do
	{
		...
		case (_action == "UseMagazine"): // placed explosive
		{
			...
			if !(_nearbyStores isEqualTo []) exitWith
			{
				...
				[format ["Vous n'êtes pas autorisé à poser des explosifs à %1m du magasin.", _minDist], 5] call mf_notify_client;
				_handled = true;
			};
			...
			{
				playSound "FD_CP_Not_Clear_F";
				[format ["Vous n'êtes pas autorisé à placer des explosifs à %1m d'une mission.", _minDist], 5] call mf_notify_client;
				_handled = true;
			};
		};
		...
		{
			playSound "FD_CP_Not_Clear_F";
			[format ["Vous n'êtes pas autorisé à démonter les armes.\nUtilisez l'option ""%1"" à la place.", ["STR_R3F_LOG_action_deplacer_objet", "Move"] call getPublicVar], 5] call mf_notify_client;
			_handled = true;
		};
