...
if (mutexScriptInProgress) exitWith {
	titleText ["Vous faites déjà autre chose.", "PLAIN DOWN", 0.5];
};
...
_resupplyThread = [_vehicle, _unit] spawn
{
	...
	_checkAbortConditions =
	{
		...
		call
		{
			...
			{
				doCancelAction = false;
				_abortText = "Annulé par le joueur.";
			};

			if (!alive player) exitWith
			{
				_abortText = "Vous êtes mort.";
			};
			...
			{
				_pauseText = "Reprend le contrôle d'un autre véhicule.";
				_abortText = "Un autre joueur a pris le controle du véhicule.";
			};
			...
			if (call _checkCondition) exitWith
			{
				_abortText = "Le véhicule a été détruit.";
			};
			...
			if (call _checkCondition) exitWith
			{
				_pauseText = "Approchez vous plus près du véhicule.";
				_abortText = "Trop loin du véhicule.";
			};
			...
			{
				_pauseText = "Remontez dans le véhicule.";
				_abortText = "Vous n'êtes pas dans un véhicule.";
			};
			...
			{
				_pauseText = "Le siège tireur doit être vide.";
				_abortText = "Quelqu'un est déjà en tireur.";
			};
		};
		...
		if (_abortText != "") then
		{
			titleText [format ["%1\n%2", _abortText, "Ravitaillement annulé"], "PLAIN DOWN", 0.5];
	...
	_checkPlayerMoney =
	{
		...
		{
			_text = format ["%1\n%2", format ["Pas assez d'argent , vous avez besoin de $%1 pour ravitailler %2", _price, _vehName], "Resupply sequence aborted"];
	...
	call
	{
		...
		{
			_text = format ["%1\n%2", "Le raviltaillement a commencé", "Veuillez sortir."];
		...
		if (player getVariable ["cmoney",0] >= _price) then
		{
			_msg = format ["%1<br/><br/>%2", format ["Celà vous coûtera $%1 pour ravitailler %2.", _price, _vehName], "Do you want to proceed?"];
		...
		_text = format ["%1\n%2", format ["Vous avez payé $%1 pour ravitailler %2.", _price, _vehName], "Please stand by..."];
		[_text, 10] call mf_notify_client;
		...
		// Reload turret mags
		{
...

			{
				...
				if (_notFull) then
				{
					...
					_text = format ["Rechargement %1...", [_vehName, _magName] select (_magName != "")];
		...
		if (fuel _vehicle < 0.999 && !_isStaticWep) then
		{
			while {fuel _vehicle < 0.999} do
			{
				...
				"Ravitaillement..." call _titleText;
		...
		titleText ["Votre véhicule est ravitaillé.", "PLAIN DOWN", 0.5];
	};
};
