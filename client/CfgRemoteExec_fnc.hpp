...
//added by [utopia] Amaury for leveling system
class lvl_addXP {};
class lvl_removeXP {};

// do NOT whitelist BIS_fnc_execVM or BIS_fnc_spawn, it will allow exploits!

class fn_debugBoxAddTicket { allowedTargets = 2;};

class fn_updateBaseMenu {};
class fn_addBaseMoney {allowedTargets = 2;};

class fn_sendBaseMenu { allowedTargets = 2; };
class fn_sendServicesMenu { allowedTargets = 2;};
class fn_updateServicesMenu {};

class fn_debugBoxRequest {allowedTargets = 2;};
class fn_updateDebugBox {};

class fn_artilleryRequest { allowedTargets = 2; };
class fn_mortarRequest { allowedTargets = 2; };
