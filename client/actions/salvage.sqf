...
if (_success) then
{
	...
	if("recuperation" call lvl_haveCapacity) then {
		_metalQtty = _metalQtty * 2;
	};

	for "_i" from 1 to _metalQtty do {
		if(["metal",1] call mf_inventory_can_add) then {
		["metal",1] call mf_inventory_add;
		}else{
		_veh = "Land_MetalWire_F" createVehicle (position player);
		_veh setPos (getpos player);
		_veh setVariable ["mf_item_id","metal",true];
		};
	};

	[format ["You have obtained $%1 from salvaging and %2 piece(s) of metal.", [_money] call fn_numbersText,_metalQtty], 5] call mf_notify_client;
