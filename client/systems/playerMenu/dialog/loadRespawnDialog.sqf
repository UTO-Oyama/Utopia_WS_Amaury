...
_respawnText ctrlSetStructuredText parseText (format ["Bienvenue sur Utopia Gaming A3Wasteland<br/>Vous êtes en %1. Veuillez sélectionner un endroit de réapparition.", _side]);
	...
	if (!isNil "_location") then
	{
		...
		else
		{
			if (_location != "") then
			{
				...
				{
					_textStr = _textStr + "[<t color='#ff0000'>Bloqué par des ennemis.</t>] ";
				}
				else
				{
		...
		if (_isValid) then
		{
			...
			if (_friendlyPlayers > 0) then
			{
				...
				_extraTextStr = _extraTextStr + format ["<t color='#00ff00'>%1 joueur(s) alliés</t>", _friendlyPlayers];
			};

			if (_enemyPlayers > 0) then
			{
				if (_extraTextStr != "") then { _extraTextStr = _extraTextStr + ", " };
				_extraTextStr = _extraTextStr + format ["<t color='#ff0000'>%1 joueur(s) ennemis</t>", _enemyPlayers];
			};

			if (_enemyNPCs > 0) then
			{
				if (_extraTextStr != "") then { _extraTextStr = _extraTextStr + ", " };
				_extraTextStr = _extraTextStr + format ["<t color='#ff0000'>%1 IA(s) ennemies</t>", _enemyNPCs];
			};
