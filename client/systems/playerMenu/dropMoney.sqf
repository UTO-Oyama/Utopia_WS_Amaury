...
if(mutexScriptInProgress) exitWith {
	player globalChat "Vous faites déjà autre chose !";
};

private["_money","_pos","_cash"];
_money = parsenumber(GET_SELECTED_DATA(2100));

if((player getVariable "cmoney" < _money) OR (player getVariable "cmoney" < 0)) exitwith {hint format["Vous n'avez pas $%1 sur vous", [_money] call fn_numbersText];};
