...
storeSellingHandle = _this spawn
{
	...
	if (count _allCrateItems == 0) exitWith
	{
		if (!_forceSell) then
		{
			...
			[format ["""%1"" ne contient pas d'items valides.", _objName], "Erreur"] call BIS_fnc_guiMessage;
		};
	};
	...
	if (_forceSell) then
	{
		...
		hint format [format ["L'inventaire de ""%1"" a été vendu pour $%2", _objName, _sellValue]];
	...
	else
	{
		...
		if ([parseText _confirmMsg, "Confirmer", "Vendre", true] call BIS_fnc_guiMessage) then
		{
			...
			if (CARGO_STRING(_crate) == _originalCargo) then
			{
				...
				_hintMsg = if (_deleteObject) then { 'Vous avez vendu "%1" pour $%2' } else { "Vous avez vendu l'inventaire de ""%1"" pour $%2" };
