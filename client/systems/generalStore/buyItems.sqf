...
storePurchaseHandle = _this spawn
{
	...
	_showInsufficientFundsError =
	{
		...
		hint parseText format ["Pas assez d'argent pour <br/>""%1""", _itemText];
	...
	{
		_itemText = _this select 0;
		hint parseText format ["Pas assez de place pour <br/>""%1""", _itemText];
		playSound "FD_CP_Not_Clear_F";
		_price = -1;
	};
	...
	{
		_itemText = _this select 0;
		hint parseText format ["<t color='#ffff00'>Une erreur est servenue.</t><br/>L'achat de ""%1"" a été annulé.", _itemText];
		playSound "FD_CP_Not_Clear_F";
		_price = -1;
	};
	...
	{
		_itemText = _this select 0;
		hint format ["""%1"" a été créé dehors , devant le magasin.", _itemText];
	...
	_showReplaceConfirmMessage =
	{
		...
		{
			_itemText = format ["Acheter ces %1 va remplacer ceux déjà existants.", _itemText];
		}
		else
		{
			if (param [2, false, [false]]) then
			{
				_itemText = format ["Acheter ça %1 va remplacer celui que vous avez déjà.", _itemText];
			}
			else
			{
				_itemText = format ["Acheter ça %1 va remplacer celui que vous avez déjà , et son contenu sera perdu.", _itemText];
			};
		};

		_confirmResult = [parseText _itemText, "Confirmer", "Acheter", true] call BIS_fnc_guiMessage;
	...
	_showAlreadyHaveItemMessage =
	{
		...
		if (param [1, false, [false]]) then
		{
			_itemText = format ["Vous avez déjà des %1.", _itemText];
		}
		else
		{
			_itemText = format ["Vous avez déjà ceci : %1.", _itemText];
		};

		playSound "FD_CP_Not_Clear_F";
		_price = -1;

		[parseText _itemText, "Erreur"] call BIS_fnc_guiMessage
	};
