...
pp_retrieve_transaction_ok = {
  ...
  if (_cost > _cmoney) exitWith {
    _player groupChat format["%1, vous n'avez pas assez d'argent pour sortir %2", (name _player), ([_class] call generic_display_name)];
    false
  };
...
pp_retrieve_allowed = {
  ...
  if (isSCALAR(_wait_time) && {_wait_time > 0 }) exitWith {
    ...
    _player groupChat format["%1, vous avez besoin d'attendre %2s avant de récupérer %3", (name _player), _timer, ([_class] call generic_display_name)];
    false
  };
  ...
  if (isSCALAR(pp_retrieve_cost) && {pp_retrieve_cost > 0}) exitWith {
    ...
    _msg = format["Cela vous coûtera $%1 de récupérer %2. Voulez-vous vraiment le récupérer ?", strM(_cost), ([_class] call generic_display_name)];

    if (not([_msg, "Confirmer", "Oui", "Non"] call BIS_fnc_guiMessage)) exitWith {false};
...
pp_park_allowed = {
  ...
  if (isARRAY(pp_disallowed_vehicle_classes) && {count(pp_disallowed_vehicle_classes) > 0 && { ({_class isKindOf _x} count pp_disallowed_vehicle_classes) > 0}}) exitWith {
    _msg = format["Ce véhicule (%1) ne peut pas être parqué.", ([_class] call generic_display_name)];
  ...
  if (isSCALAR(pp_max_player_vehicles) && {pp_max_player_vehicles > 0 && {_count >= pp_max_player_vehicles}}) exitWith {
    _msg = format["Vous avez déjà %1 vehicule(s) de parqués. Il n'y plus de places disponibles.", _count];
    [_msg, "Parking complet", "ok", false] call BIS_fnc_guiMessage;
    false
  };
...
pp_park_vehicle_action = {
  ...
  def(_vehicle_id);
  _vehicle_id = ["Parquer le véhicule", _vehicles] call pp_interact_park_vehicle_wait;
  ...
  if (!isOBJECT(_vehicle)) exitWith {
    _player groupChat format["%1, le véhicule que vous avez sélectionné ne peut être trouvé.", (name _player)];
  };
  ...
  _player groupChat format["Veuillez patienter pendant que nous parquons votre %1", ([typeOf _vehicle] call generic_display_name)];
...
pp_retrieve_vehicle_action = {
  ...
  def(_vehicle_id);
  _vehicle_id = ["Récupérer le véhicule", _parked_vehicles] call pp_interact_park_vehicle_wait;
  ...
  if (!isARRAY(_vehicle_data)) exitWith {
    player groupChat format["ERREUR : Le véhicule (%1) n'a pas été trouvé", _vehicle_id];
  };
  ...
  _player groupChat format["Veuillez patienter un petit momment avant de parquer %1", ([_class] call generic_display_name)];
