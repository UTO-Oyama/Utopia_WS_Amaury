...
va_unflip_action = {
  ...
  if (isSCRIPT(va_unflip_action_script) && {not(scriptDone va_unflip_action_script)}) exitWith {
    player groupChat format["Le véhicule est déjà en train d'être retourné , veuillez patienter."];
  };
  ...
  va_unflip_action_script = _this spawn {
   //player groupChat format["va_unflip_action %1",_this];
    ...
    _player groupChat format["Retournement de %1, attendez %2 secondes.", _display_name, _sleep];
    sleep _sleep;

    if ((_player distance _vehicle) > _dist) exitWith {
      _player groupChat format["Impossible de retourner %1, Vous devez rester à %2 mètres.", _display_name, _dist];
    };

    [[_vehicle,surfaceNormal (getPos _vehicle)],"A3W_fnc_unflip",_vehicle] call BIS_fnc_MP;


    _player groupChat format["Le %1 a été retourné.", _display_name];
  };
};
...
va_information_action = {
  ...
  forEach(
    [["   ID:         ", ([_tag,17] call str_truncate)],
     ...
     ["   Altitude:   ", str(round(getposASL _vehicle select 2)) + " mètre(s) ASL"],
     ["   Conducteur: ", ([_driver,17] call str_truncate)],
     ["   Sièges:     ", str((_vehicle emptyPositions "cargo")+(_vehicle emptyPositions "driver")) + " siège(s)"],
     ["   Taille:     ", str(round((sizeOf _class)*10)/10) + " mètre(s)"],
     ["   Propriétaire:",  ([_owner,17] call str_truncate)]


    ]);

  _text = format["<t align='center' font='PuristaMedium' size='1.4' >Informations sur le véhicule</t><br /><img image='%1' size='2.8'   /><br /><t  align='center'>(%2)</t>", _picture, ([_display_name,25] call str_truncate)] + "<br /><br />" + _text;
...
va_outside_owner_add_actions = {
  ...
  {if (true) then {
    ...
    def(_action_id);
    _action_id = _player addaction [format["<img image='addons\vactions\icons\pull.paa'/> Pousser %1", (name _member)], {_this call va_pull_player_action;}, [_player, _vehicle, _member],10,false,false,"",
...
va_outside_add_actions = {
  ...
  _action_id = _player addaction [format["<img image='addons\vactions\icons\flip.paa'/> Retourner %1", _display_name], {_this call va_unflip_action;}, [_player, _vehicle],10,false,false,"",
  ...
  _action_id = player addaction [format["<img image='addons\vactions\icons\lock.paa'/> Verouiller %1", _display_name], {_this call va_lock_action;}, [_player, _vehicle],0,false,false,"",
  ...
  _action_id = player addaction [format["<img image='addons\vactions\icons\key.paa'/> Déverouiller %1", _display_name], {_this call va_unlock_action;}, [_player, _vehicle],999,true,false,"",
...
va_inside_add_actions = {
  ...
  //Add vehicle lock action
  _action_id = player addaction [format["<img image='addons\vactions\icons\lock.paa'/> Verouiller %1", _display_name], {_this call va_lock_action;}, [_player, _vehicle],10,false,false,"",
  format["([objectFromNetId %1, objectFromNetId %2] call va_lock_action_available)", str(netId _player), str(netId _vehicle)]];
  va_inside_actions = va_inside_actions + [_action_id];

  //Add vehicle unlock action
  _action_id = player addaction [format["<img image='addons\vactions\icons\key.paa'/> Déverouiller %1", _display_name], {_this call va_unlock_action;}, [_player, _vehicle],10,false,false,"",
  format["([objectFromNetId %1, objectFromNetId %2] call va_unlock_action_available)", str(netId _player), str(netId _vehicle)]];
  va_inside_actions = va_inside_actions + [_action_id];
};
