...
_max_dist = getNumber (missionConfigFile >> "basesCFG" >> "mortar_max_dist");
...
if(_hasFired) exitWith {
	["Le mortier a déjà tiré récemment."] remoteExecCall ["ug_fnc_serverMessage",_sender];
};
...
[format["<t size='1.2' align='center'>Attention</t><br/>%1 a demandé un tir de mortier de votre base.<br/> préparez vous à l'impact !",_sender]] remoteExecCall ["ug_fnc_serverMessage",(if(_side == "BLUFOR") then {west}else{east})];

[_pos,_side] spawn fn_mortarFire;