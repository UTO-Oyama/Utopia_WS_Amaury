...
_setupObjects =
{
	...
	_vehDeterminer = if ("AEIMO" find (_vehicleName select [0,1]) != -1) then { "Un" } else { "Un" };

	_missionHintText = format ["%1 <t color='%3'>%2</t> a été immobilisé , allez le récupérer !", _vehDeterminer, _vehicleName, mainMissionColor];
};
...
_successExec =
{
	...
	_successHintMessage = format ["Le %1 a été capturé , bien joué !", _vehicleName];
};
